...
class ListDataset(Dataset):
    def __init__(self, list_path, img_size=416):
        ...
        if('test_images' in self.img_files[0]):
            ...
            self.label_files = [path.replace('train_images', 'train_labels').replace('.jpg', '.txt') for path in self.img_files]
            self.img_files = [path.replace('train_images', 'images') for path in self.img_files]
        self.img_shape = (img_size, img_size)
        self.max_objects = 1
